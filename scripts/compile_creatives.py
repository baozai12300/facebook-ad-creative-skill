...
PLACEMENTS = {
    "feed": ("4:5", "1080x1350", "center 80%", "Keep product and headline away from extreme edges."),
    "square_feed": ("1:1", "1080x1080", "centered safe field", "Recompose natively for square; do not crop a 4:5 layout."),
    "story": ("9:16", "1080x1920", "middle 70%", "Keep key content away from top and bottom UI regions."),
    "reels": ("9:16", "1080x1920", "upper-middle field", "Keep the lower region quiet for controls."),
    "carousel": ("1:1", "1080x1080", "centered safe field", "Use one message per card and keep the visual system consistent."),
}
...
def choose_angles(data: Dict[str, Any]) -> List[Dict[str, str]]:
    angles = [_custom_angle(name) for name in data["angle_preferences"]]
    core = [
...
def build_copy(data: Dict[str, Any], angle: Dict[str, str], benefit: str) -> Dict[str, Any]:
    ...
    headlines = {
        "clarity": f"Meet {data['product_name']}",
        "problem": f"A simpler way to get {benefit}",
        "lifestyle": "Made for your daily routine",
        "benefit": benefit,
        "comparison": f"Make {benefit} easier",
        "proof": "Why customers choose it",
        "offer": data["offer_info"],
    }
    support = data["product_description"].rstrip(".") if mode in {"standard", "heavy"} else ""
    callouts = data["key_features"][:2] if mode in {"standard", "heavy"} else []
    cta = ""
    if mode == "heavy" and data["cta_mode"] != "none":
        cta = "Shop Now" if data["cta_mode"] == "direct" else "Learn More"
    return {"headline": headlines.get(angle["kind"], f"Discover {data['product_name']}"), "support": support, "callouts": callouts, "cta": cta}
...
def select_scene(data: Dict[str, Any], angle: Dict[str, str], index: int) -> str:
    if data["scene_preferences"]:
        return cycle(data["scene_preferences"], index)
    category = data["product_category"].lower()
...
def quality_gate(data: Dict[str, Any], plan: Dict[str, Any], prompt: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "checks": ["product fidelity", "one-glance message", "product prominence", "layout clarity", "copy accuracy", "placement fit", "claim integrity"],
    }
...
def build_creatives(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for index in range(data["generation_count"]):
        ...
        dna_values = VISUAL_DNA[angle["style"]]
    ...
    findings = []
    if len(creatives) >= 4 and angle_count < 4:
        findings.append("fewer than four distinct dominant angles")
    if len(creatives) >= 4 and layout_count < 3:
        findings.append("fewer than three layout families")
    if combo_count != len(creatives):
